...
export async function createDoctor(_prevState: any, formData: FormData) {
  try {
    ...
    const response = await serverFetchClient.post("/user/doctor-create", {
      body: newFormData,
      credentials: "include",
    });
...
export async function getDoctors(queryString?: string) {
  try {
    const response = await serverFetchClient.get(
...
export async function getDoctorById(id: string) {
  try {
    const response = await serverFetchClient.get(`/doctor/${id}`);
    const result = await response.json();
    return result;
  } catch (error: any) {
    console.log(error);
...
export async function updateDoctor(
  ...
  try {
    ...
    const response = await serverFetchClient.patch(`/doctor/${id}`, {
      headers: {
...
export async function softDeleteDoctor(id: string) {
  try {
    const response = await serverFetchClient.delete(`/doctor/soft/${id}`);
    const result = await response.json();
...
export async function deleteDoctor(id: string) {
  try {
    const response = await serverFetchClient.delete(`/doctor/${id}`);
    const result = await response.json();
